...
export default function TrimDetailsPage({ params }: PageProps) {
  const { clientId } = use(params);
  const [trimColors, setTrimColors] = useState<TrimColor[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const { saveField, getSaveStatus } = useRowAutoSave<TrimColor>({
    onSave: async (colorId, field, value) => {
      await fetch(`/api/clients/${clientId}/trim-colors/${colorId}`, {
        method: "PATCH",
  ...
  const loadData = async () => {
    try {
      const response = await fetch(`/api/clients/${clientId}/trim-colors`);
      setTrimColors(await response.json());
    } catch (error) {
      console.error("Error loading trim colors:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const trimTypeOptions = [
    ...
    { value: "Vinyl Trim", label: "Vinyl Trim" },
    { value: "Aluminum Trim", label: "Aluminum Trim" },
  ];
  ...
  const addColor = async () => {
    try {
      ...
      setTrimColors((prev) => [...prev, newColor]);
    } catch (error) {
      console.error("Error adding trim color:", error);
    }
  };

  const deleteColor = async (colorId: string) => {
    if (!confirm("Are you sure you want to delete this trim color?")) return;
    
    try {
      await fetch(`/api/clients/${clientId}/trim-colors/${colorId}`, {
        method: "DELETE",
      });
      setTrimColors((prev) => prev.filter((color) => color.id !== colorId));
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading trim colors...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-full mx-auto p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">F. Trim Details</h1>
            <p className="text-gray-600">Configure trim color options.</p>
          </div>
          <button
            onClick={addColor}
      ...
      {trimColors.length === 0 ? (
        <div className="bg-white rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No trim colors yet</h3>
          <p className="text-gray-600 mb-6">Add your first trim color option.</p>
          <button
            onClick={addColor}
              ...
              <tbody className="divide-y divide-gray-200">
                {trimColors.map((color) => (
                  <tr key={color.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <EditableSelectField
                        value={color.trimType || "Wood Trim"}
                        options={trimTypeOptions}
                        onChange={(value: string) => saveField(color.id, "trimType", value)}
                        className="w-36"
                      />
                    </td>
                    <td className="px-4 py-3">
